/**
 * LoginPage — Authentication with inline domain selection.
 * Users choose their workspace domain before entering the app.
 */
import { useState } from 'react'
import { useNavigate, NavLink } from 'react-router-dom'
import { motion, AnimatePresence } from 'framer-motion'
import { ArrowRight, Mail, Lock, CheckCircle, Sparkles, Globe, Cpu, LayoutDashboard, Shield } from 'lucide-react'
        ...
        >
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-[var(--text-primary)] tracking-tight mb-4">
              {step === 'credentials' ? 'Welcome back to ' : 'Choose your industry for '}
            ...
            <p className="text-[var(--text-secondary)] text-sm leading-relaxed mb-10 max-w-sm">
              {step === 'credentials'
                ? 'Sign in below to continue into your semantic intelligence workspace.'
                ...
                >
                  ...
                  <div className="pt-4 flex flex-col sm:flex-row gap-3">
                    <button type="submit" className="btn-primary flex-1 py-3.5 text-sm flex items-center justify-center gap-2 group">
                      Login to Workspace <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                    </button>
                  ...
                  <div className="mt-8 pt-6 border-t border-[var(--border-subtle)]">
                    <p className="text-sm text-[var(--text-muted)]">
                      Need an account?{' '}
                      <NavLink to="/register" className="text-[var(--aurora)] hover:text-[var(--plasma)] transition-colors font-semibold">
                        Register here
                      </NavLink>
                    </p>
                  </div>
                      ...
                      return (
                        <button
                          ...
                          className={`card p-4 flex flex-col items-start gap-2 transition-all duration-200 hover:-translate-y-0.5 text-left group relative ${
                            isSelected ? 'border-[var(--aurora)] shadow-[0_0_15px_rgba(212,149,106,0.2)]' : 'hover:border-[var(--aurora)]/50'
                          }`}
                        >
                          {isSelected && (
