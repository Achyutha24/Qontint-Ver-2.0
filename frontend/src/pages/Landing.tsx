...
import {
  ArrowRight, Network, Cpu, Shield, Zap, Search, Globe, Database,
  Building2, CreditCard, HeartPulse, Cloud, ShoppingCart, GraduationCap,
  Factory, Megaphone, ShieldCheck, Home, Laptop, Sparkles, CheckCircle, Briefcase
} from 'lucide-react'
...
function HomeHeroScene() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < numParticles; i++) {
      const z = isDark
        ? (i < numParticles * 0.35 ? Math.random() * 3 : (i < numParticles * 0.7 ? Math.random() * -4 - 2 : Math.random() * -6 - 6))
        : (Math.random() - 0.5) * 15
    ...
    const latticeMat = new THREE.MeshBasicMaterial({ color: 0xd4956a, wireframe: true, transparent: true, opacity: 0.4, blending: THREE.AdditiveBlending })
    ...
    const pulseMat = new THREE.ShaderMaterial({
      uniforms: { uTime: { value: 0 }, uColor: { value: new THREE.Color(0xd4956a) } },
    ...
    for (let i = 0; i < beamCount; i++) {
      ...
      const beamMat = new THREE.LineBasicMaterial({ color: 0xd4956a, transparent: true, opacity: 0.1, blending: THREE.AdditiveBlending })
    ...
    for (let i = 0; i < 8; i++) {
      ...
      orbit.rotation.z = (i / 8) * Math.PI * 2
    ...
    if (isDark && !isMobile) {
      ...
      composer.addPass(new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), isDark ? 0.8 : 0.2, 0.4, 0.85))
    }
...
export default function Landing() {
  ...
  // Handle scroll-to-section from state passed by LandingNavbar
  useEffect(() => {
  ...
  return (
    <div className="text-[var(--text-primary)] transition-colors duration-300">
      ...
      <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-6 text-center mt-16"
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
            ...
            <button className="btn-primary w-full sm:w-auto px-10 py-4 text-lg flex items-center justify-center gap-2" onClick={handleGetStarted}>
              Analyze Content <ArrowRight className="w-5 h-5" />
            </button>
            <button className="btn-secondary w-full sm:w-auto px-10 py-4 text-lg flex items-center justify-center gap-2" onClick={() => navigate('/app/graph')}>
      ...
      <section id="workspace" className="py-12 container mx-auto px-6 relative z-10">
        <MotionWrapper className="max-w-6xl mx-auto">
          {!domainContext.hasSelectedWorkspace ? (
            ...
            <div className="card p-5 max-w-md mx-auto flex items-center justify-between border-[var(--aurora)]/30 bg-gradient-to-r from-[var(--bg-depth)] to-transparent shadow-[0_0_20px_rgba(212,149,106,0.05)]">
              ...
              <button
                onClick={() => domainContext.resetWorkspace()}
